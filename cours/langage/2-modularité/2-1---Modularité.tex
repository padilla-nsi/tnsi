...
\tightlist
\item
  ils font la même chose, mais avec des stratégies complètement
  différentes
\item
...
-\texttt{s} représente d'une manière ou d'une autre un ensemble de dates
qu'il faut \emph{créer}. - Il faut vérifier si \texttt{s}
\emph{contient} l'élément \texttt{x}. - Il faut être capable
d'\emph{ajouter} l'élément \texttt{x} à \texttt{s} si besoin.
...
\subsection{1.4 Interfaces}\label{interfaces}}
