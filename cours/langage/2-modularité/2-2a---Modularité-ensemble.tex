...
\setcounter{secnumdepth}{0} % Turns off numbering for sections
    ...
    \geometry{a4paper,tmargin=3cm,bmargin=2cm,lmargin=1cm,rmargin=1cm}\fancyhead[L]{Thème à définir}\fancyhead[L]{\adjustimage{height=1cm, valign=m}{/home/bouscadilla/Documents/Code/nbconvert/template/latex/pdf_solution/papier_eleve_ico_langage}\ttfamily\scshape Langage}\fancyhead[C]{\bfseries\MakeUppercase{2-2a---Modularité-ensemble}}\fancyhead[C]{\bfseries\MakeUppercase{2 --- Modularité}}\fancyhead[R]{\monthyeardate\today}
...
anniversaires}\label{un-exemple-le-paradoxe-des-anniversaires}}
    ...
    \hypertarget{groupe-1-les-ensembles}{%
\subsubsection{Groupe 1 --- les
ensembles}\label{groupe-1-les-ensembles}}

    Connais-tu le \textbf{paradoxe des anniversaires} ? Lorsqu'on réunit 23
personnes, il y a une chance sur deux pour que deux d'entre elles soient
nées le même jour.

Écrivons un programme qui vérifie expérimentalement ce paradoxe.

On va répéter de nombreuses fois un tirage aléatoire de 23 dates
d'anniversaire, c'est-à-dire 23 entiers entre 1 et 365, on va observer
combien de fois au moins deux personnes sont nées el même jour.
...
\end{tcolorbox}
    ...
    \begin{Verbatim}[commandchars=\\\{\}]
501 doublons sur 1000 tirages
fréquence :  0.501
    \end{Verbatim}

    \emph{L'\textbf{objectif} de cette activité est de présenter la fonction
\texttt{contient\_doublon(t)} à vos camarades.} Afin de vous aider à
vous approprier la problématique, voici une liste de questions :

\textbf{Analyse du programme 1 :}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  À quoi sert la boucle \texttt{while\ n\ \textless{}\ 1000} ?
\item
  Que pensez-vous de l'affirmation du paradoxe des anniversaires ?
\end{enumerate}

\textbf{Utilité de la fonction \texttt{contient\_doublon(t)}:}
...
\tightlist
\item
  Dans la fonction \texttt{contient\_doublon(t)}, quel est le type
  Python de la variable \texttt{t}?
\item
  Que renvoie la fonction \texttt{contient\_doublon(t)} ?
\item
  Comment fonctionne la ligne \texttt{if\ contient\_doublon(t)} ?
\item
  Quel est l'intérêt de la fonction \texttt{contient\_doublon(t)} ?
\end{enumerate}

\textbf{Fonctionnement de la fonction \texttt{contient\_doublon(t)} :}
...
\tightlist
\item
  Quel est le type Python de la variable \texttt{s} ?
\item
  Quel est le but de la commande \texttt{s.add(x)} ?
\item
  Quel est l'effet de la ligne \texttt{for\ x\ in\ t:} ?
\item
  L'instruction \texttt{if\ x\ in\ s:} est-elle immédiate ?
\end{enumerate}
