...
\setcounter{secnumdepth}{0} % Turns off numbering for sections
    ...
    \geometry{a4paper,tmargin=3cm,bmargin=2cm,lmargin=1cm,rmargin=1cm}\fancyhead[L]{Thème à définir}\fancyhead[L]{\adjustimage{height=1cm, valign=m}{/home/bouscadilla/Documents/Code/nbconvert/template/latex/pdf_solution/papier_eleve_ico_ds}}\fancyhead[C]{\bfseries\MakeUppercase{2021J1ME2-ex1}}\fancyhead[C]{\bfseries\MakeUppercase{Correction - 2021J1ME2 - ex1}}\fancyhead[R]{\monthyeardate\today}
...
\subsection{Exercice 1}\label{exercice-1}}
    ...
    1 - La relation \texttt{Role} possède une clé primaire composée du
triplet \((\texttt{\#idPiece, \#idActeur, \#nomRole})\). Il est
obligatoire de posséder une clé primaire lorsque l'on insère une entrée.
Or le triplet est composé de deux clés étrangères.

\textbf{Si} les relations \texttt{Piece} et \texttt{Acteur} sont vides,
\textbf{alors} le triplet ne peut pas être défini car l'unicité de la
clé primaire n'est plus assurée. Le système de base de donnée lève alors
une exception.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{INSERT} \KeywordTok{INTO} \KeywordTok{Role}\NormalTok{(\#idPiece, \#idActeur, nomRole) }\KeywordTok{VALUES}
\NormalTok{    (}\DecValTok{46721}\NormalTok{, }\DecValTok{389761}\NormalTok{, }\StringTok{\textquotesingle{}Tartuffe\textquotesingle{}}\NormalTok{);}
\end{Highlighting}
\end{Shaded}

    3- Cette requête met à jour certaines entrées de la relation
\texttt{Piece}. Toutes les entrées qui possèdent les valeurs
\texttt{"Américain"} ou \texttt{"Britannique"} pour l'attribut
\texttt{langue} voient ces valeurs changer pour \texttt{"Anglais"}.

    4.a - Nom et prénom des artistes nés après 1990 :

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{SELECT}\NormalTok{ nom, prenom}
\KeywordTok{FROM}\NormalTok{ Acteur}
\KeywordTok{WHERE}\NormalTok{ anneeNaiss }\OperatorTok{\textgreater{}} \DecValTok{1990}\NormalTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{SELECT} \FunctionTok{MIN}\NormalTok{(annee)}
\KeywordTok{FROM}\NormalTok{ Acteur;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{SELECT} \KeywordTok{DISTINCT}\NormalTok{ R.nomRole}
\KeywordTok{FROM}\NormalTok{ Acteur A}
\KeywordTok{JOIN} \KeywordTok{Role}\NormalTok{ R }\KeywordTok{ON}\NormalTok{ A.idActeur }\OperatorTok{=}\NormalTok{ R.idActeur}
\KeywordTok{WHERE}\NormalTok{ A.nom }\OperatorTok{=} \OtherTok{"Macaigne"} \KeywordTok{and}\NormalTok{ A.prenom }\OperatorTok{=} \OtherTok{"Vincent"}\NormalTok{;}
\end{Highlighting}
\end{Shaded}

    4.d - Le titre des pièces écrites en Russe dans lesquelles l'actrice
Jeanne Balibar a joué :

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{SELECT}\NormalTok{ P.titre}
\KeywordTok{FROM}\NormalTok{ Piece P}
\KeywordTok{JOIN} \KeywordTok{Role}\NormalTok{ R }\KeywordTok{on}\NormalTok{ P.idPiece }\OperatorTok{=}\NormalTok{ R.idPiece}
\KeywordTok{JOIN}\NormalTok{ Acteur A }\KeywordTok{ON}\NormalTok{ R.idActeur }\OperatorTok{=}\NormalTok{ A.idActeur}
\KeywordTok{WHERE}\NormalTok{ P.langue }\OperatorTok{=} \OtherTok{"Russe"}
    \KeywordTok{AND}\NormalTok{ A.nom }\OperatorTok{=} \OtherTok{"Balibar"}
    \KeywordTok{AND}\NormalTok{ A.prenom }\OperatorTok{=} \OtherTok{"Jeanne"}\NormalTok{;}
\end{Highlighting}
\end{Shaded}
